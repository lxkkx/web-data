...
const RegistrationScreen = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setError(typeof detail === 'string' ? detail : (detail ? JSON.stringify(detail) : 'Registration failed. Please check your details.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#FEFEFE] flex flex-col font-sans">
      ...
      <div className="flex-1 flex flex-col px-8 pt-4 w-full max-w-md mx-auto relative">
        <h2 className="text-[32px] font-extrabold text-gray-900 leading-tight">Create Account</h2>
        ...
        <form onSubmit={handleRegister} className="flex flex-col flex-1">
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full h-[60px] mt-8 rounded-[16px] bg-[#6750A4] text-white flex items-center justify-center shadow-md active:bg-[#4F378B] hover:bg-[#594099] transition-colors disabled:opacity-50"
          >
            {loading ? (
              <Loader2 size={24} className="animate-spin" />
            ) : (
              <span className="text-[18px] font-bold">Register</span>
            )}
          </button>

          <div className="mt-8 flex items-center justify-center gap-1 pb-10">
            <span className="text-[14px] font-medium text-gray-500">Already have an account?</span>
            <Link to="/login" className="text-[14px] font-bold text-[#6750A4] hover:opacity-80 transition-opacity">
              Login
            </Link>
          </div>
        </form>
